#!/usr/bin/env python3
"""Check the content-summarizer Layer 3 compression ratio against the raw source.
...
Prints byte sizes, ratio, and status for:
    - Layer 3 (Educational Reading Notes) roughly between 20% and 60% of raw

Outside the range is a warning, not a failure.
Layer 3 below 20% is a coverage warning: run the coverage audit and add only
missing effective information.
Layer 3 above 60% is a compression warning: tighten only when doing so preserves
...
def main():
    if len(sys.argv) != 3:
        print(f"Usage: {sys.argv[0]} summary.md raw.md", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Layer 3: {layer3_bytes:>6d} / {raw_bytes:>6d} = {l3_pct:5.1f}%  {l3_status}")
    if l3_status == 'WARN':
        if l3_pct < 20:
            print("Layer 3 below 20%: run the coverage audit; expand only missing effective information.")
        else:
            print("Layer 3 above 60%: tighten if possible while preserving effective information.")
